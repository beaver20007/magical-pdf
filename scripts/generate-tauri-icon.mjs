...
await generatePlatformIcons(sourceIcon);
...
async function generatePlatformIcons(sourceIcon) {
  if (process.platform !== "darwin") {
    const png32 = encodePng(32, 32, resizeNearest(raw, size, 32));
    const png128 = encodePng(128, 128, resizeNearest(raw, size, 128));
    const png256 = encodePng(256, 256, resizeNearest(raw, size, 256));

    await writeFile(resolve(iconDir, "32x32.png"), png32);
    await writeFile(resolve(iconDir, "128x128.png"), png128);
    await writeFile(resolve(iconDir, "128x128@2x.png"), png256);
    await writeFile(resolve(iconDir, "icon.ico"), encodeIco([
      [32, png32],
      [128, png128],
      [256, png256],
    ]));
...
function resizeNearest(source, sourceSize, targetSize) {
  const target = Buffer.alloc((targetSize * bytesPerPixel + 1) * targetSize);

  for (let y = 0; y < targetSize; y += 1) {
    const targetRowStart = y * (targetSize * bytesPerPixel + 1);
    const sourceY = Math.min(sourceSize - 1, Math.floor((y / targetSize) * sourceSize));
    const sourceRowStart = sourceY * (sourceSize * bytesPerPixel + 1);
    target[targetRowStart] = 0;

    for (let x = 0; x < targetSize; x += 1) {
      const sourceX = Math.min(sourceSize - 1, Math.floor((x / targetSize) * sourceSize));
      const sourceOffset = sourceRowStart + 1 + sourceX * bytesPerPixel;
      const targetOffset = targetRowStart + 1 + x * bytesPerPixel;
...
function encodeIco(images) {
  ...
  for (const [imageSize, imageData] of images) {
